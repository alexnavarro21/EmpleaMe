...
function perfilLink(r) {
  ...
  if (r.referencia_rol === "empresa") return `/empresa-publica/${r.referencia_id}`;
  return null;
}

export default function SlepReportes() {
  ...
  const handleAccion = async (id, estado) => {
    setActualizando(id);
    try {
      await actualizarSlepReporte(id, estado);
  ...
  const handleEliminarYResolver = async (id, tipo) => {
    ...
    setEliminando(id);
    try {
      await eliminarContenidoSlepReporte(id);
      setReportes((prev) => prev.filter((r) => r.id !== id));
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Reportes sobre publicaciones y perfiles de empresas y colegios"
      />
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {r.referencia_rol && (
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        r.referencia_rol === "empresa"
                          ? isDark ? "bg-orange-500/15 text-orange-400" : "bg-orange-50 text-orange-700"
                          : isDark ? "bg-blue-500/15 text-blue-400" : "bg-blue-50 text-blue-700"
                      }`}>
                        {r.referencia_rol === "empresa" ? "Empresa" : "Colegio"}
                      </span>
                    )}
                  </div>
                  ...
                  <p className={`text-xs ${M} mt-1.5`}>{tiempoRelativo(r.creado_en)}</p>
                </div>
