...
export default function BuscarPerfiles() {
  ...
  // Detect role from URL to build correct candidate link
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <Icon
                  icon="mdi:search"
                  width={16}
                  className={`absolute left-3 top-1/2 -translate-y-1/2 ${M}`}
                />
                <input
                  type="text"
            ...
            <div className="mb-4">
              ...
              {uniqueCareers.map((c) => {
                ...
                const isActive =
                  selectedCareer === c ||
                  (selectedCareer !== "Todas" &&
                    (careerDisplay[selectedCareer] || selectedCareer) === label);
                return (
                  <button
                    key={c}
        ...
        <div className="col-span-3">
          <div className="grid grid-cols-2 gap-4">
            {filtered.map((s) => {
              ...
              return (
                <Card
                  key={s.usuario_id}
                  className="hover:border-[#378ADD] transition-colors"
                >
                  <div className="flex items-start gap-3 mb-3">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${S}`}
                    >
                      <Icon
                  ...
                  {s.habilidades && s.habilidades.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {s.habilidades.slice(0, 3).map((sk) => (
                        <Badge key={sk} color="blue">
                          {sk}
                        </Badge>
                      ))}
                      {s.habilidades.length > 3 && (
                        <span className={`text-xs ${M}`}>
                          +{s.habilidades.length - 3} más
                        </span>
                      )}
                    </div>
                  )}
          ...
          {filtered.length === 0 && (
            <div className={`text-center py-16 ${M}`}>
              ...
              <p className={`text-base font-medium ${T}`}>
